...
class CameraViewController: UIViewController {
  ...
  private var currentDetector: DetectorType = .detectorObjectsSingleWithClassifier
  ...
  func detectObjects(
    ...
    if modelManager.isModelDownloaded(remoteModel) {
      ...
      guard
        let localModelFilePath = Bundle.main.path(
          forResource: "automl_labeler_model",
          ofType: "tflite"
        )
      else {
        print(
    ...
    DispatchQueue.main.sync {
      ...
      for object in objects {
        ...
        var description = ""
          let jsonData = readLocalJSONFile(forName: "UK")
          if let data = jsonData {
          if let sampleRecordObj = parse(jsonData: data) {
            //You can read sampleRecordObj just like below.
              let datum = self.pullKey(key: object.labels[0].text, json: sampleRecordObj.data)
              description += "\(datum?.id.capitalized ?? "")\n"
              description += "Type: \(datum?.type ?? "")\n"
              description += "\(datum?.emissions[0] ?? ""): \(datum?.emissions[1] ?? "")"
              
              label.text = description
              label.numberOfLines = 0
              label.adjustsFontSizeToFitWidth = true
              self.url = datum?.link ?? "https://www.google.com"
              let tap = UITapGestureRecognizer(target: self, action: #selector(CameraViewController.tapFunction))
                strongSelf.annotationOverlayView.isUserInteractionEnabled = true
                strongSelf.annotationOverlayView.addGestureRecognizer(tap)
    ...
    func pullKey(key: String, json: [Datum]) -> Datum? {
        for datum in json {
            if datum.id == key {
                return datum
            }
    ...
    func parse(jsonData: Data) -> Welcome? {
        do {
            let decodedData = try JSONDecoder().decode(Welcome.self, from: jsonData)
            return decodedData
        } catch {
            print("error: \(error)")
        }
